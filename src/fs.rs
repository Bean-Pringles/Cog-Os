...
// Simple fixed-size block-based in-memory FS
pub const MAX_FILES: usize = 16;
pub const FILENAME_LEN: usize = 16;
const BLOCK_SIZE: usize = 256; // bytes per block
const BLOCKS: usize = 64; // total blocks -> total storage = 16 KiB
...
pub struct SimpleFs {
    ...
    block_map: [bool; BLOCKS], // true = used
    data: [[u8; BLOCK_SIZE]; BLOCKS],
}
...
impl SimpleFs {
    ...
    pub fn delete(&mut self, name: &str) -> Result<()> {
        ...
        // FIX: Copy data needed before mutable borrow ends
    ...
    // If file exists, free old blocks first
    if let Some(idx) = self.find_entry(name) {
        // Extract blocks and blocks_used before mutable borrow ends
    ...
    // Allocate blocks before borrowing the entry mutably again
    let allocated = self.allocate_blocks(blocks_needed)?;

    // Now borrow the file entry mutably again to write data and update block info
    ...
    for i in 0..blocks_needed {
        ...
        // Clear the block before writing new data
        for byte in self.data[block_idx].iter_mut() {
            *byte = 0;
        }

        // Copy data slice into block
    ...
    pub fn list_files(&self, out: &mut [(Option<heapless::String<{FILENAME_LEN}>>, usize); MAX_FILES]) {
        // Use heapless String to store owned names safely
        use heapless::String;
